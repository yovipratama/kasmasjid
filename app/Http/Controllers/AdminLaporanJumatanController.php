<?php namespace App\Http\Controllers;
	...
	class AdminLaporanJumatanController extends \crocodicstudio\crudbooster\controllers\CBController {
		...
		public function getIndex() {
			...
			$data['dm'] = DB::select("select SUM(jumlah) jumlah,kotak_harian,
(CASE WHEN (weekday(waktu)<=3) THEN date(waktu + INTERVAL (3-weekday(waktu)) DAY)
    ELSE date(waktu + INTERVAL (3+7-weekday(waktu)) DAY)
   END) as week_days
from masuk_dana GROUP BY week_days,kotak_harian");
			$data['dk'] = DB::select("select SUM(jumlah) jumlah,
(CASE WHEN (weekday(waktu)<=3) THEN date(waktu + INTERVAL (3-weekday(waktu)) DAY)
    ELSE date(waktu + INTERVAL (3+7-weekday(waktu)) DAY)
   END) as week_days
from keluar_dana GROUP BY week_days ORDER BY id");
...
(CASE WHEN (weekday(waktu)<=3) THEN date(waktu + INTERVAL (3-weekday(waktu)) DAY)
    ELSE date(waktu + INTERVAL (3+7-weekday(waktu)) DAY)
   END) as week_days from masuk_bahan mb group by week_days,mb.id_bahan");
...
(CASE WHEN (weekday(waktu)<=3) THEN date(waktu + INTERVAL (3-weekday(waktu)) DAY)
    ELSE date(waktu + INTERVAL (3+7-weekday(waktu)) DAY)
   END) as week_days from keluar_bahan kb group by week_days,kb.id_bahan");
			$data['waktu'] = DB::select("select (CASE WHEN (weekday(waktu)<=3) THEN date(waktu + INTERVAL (3-weekday(waktu)) DAY)
    ELSE date(waktu + INTERVAL (3+7-weekday(waktu)) DAY)
   END) as week_days from masuk_dana union select (CASE WHEN (weekday(waktu)<=3) THEN date(waktu + INTERVAL (3-weekday(waktu)) DAY)
    ELSE date(waktu + INTERVAL (3+7-weekday(waktu)) DAY)
   END) as week_days from keluar_dana ORDER BY week_days asc");
			...
			$this->cbView('laporanjumatan',$data);
		}
...
(CASE WHEN (weekday(waktu)<=3) THEN date(waktu + INTERVAL (3-weekday(waktu)) DAY)
   ...
   END) as week_days from keluar_dana ORDER BY week_days asc");
			...
			switch($filetype) {
				case "print":
					echo $view = view('xlaporanjumatan',$response)->render();
				break;
				case "pdf":
					$view = view('xlaporanjumatan',$response)->render();

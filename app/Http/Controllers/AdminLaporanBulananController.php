<?php namespace App\Http\Controllers;
	...
	class AdminLaporanBulananController extends \crocodicstudio\crudbooster\controllers\CBController {
		...
		public function getIndex() {
			...
			$data['dm'] = DB::select("select concat(MONTHNAME(waktu),' ',year(waktu)) waktu,SUM(jumlah) jumlah,kotak_harian from masuk_dana GROUP BY concat(MONTHNAME(waktu),' ',year(waktu)),kotak_harian");
			$data['dk'] = DB::select("select concat(MONTHNAME(waktu),' ',year(waktu)) waktu,SUM(jumlah) jumlah from keluar_dana GROUP BY concat(MONTHNAME(waktu),' ',year(waktu))");
			$data['bm'] = DB::select("select concat(MONTHNAME(waktu),' ',year(waktu)) waktu,sum(mb.jumlah) jumlah,(select nama from bahan where id = mb.id_bahan) bahan,(select s.nama from satuan s join bahan b on b.id_satuan = s.id where b.id = mb.id_bahan) satuan from masuk_bahan mb group by concat(MONTHNAME(waktu),' ',year(waktu)),mb.id_bahan");
			$data['bk'] = DB::select("select concat(MONTHNAME(waktu),' ',year(waktu)) waktu,sum(kb.jumlah) jumlah,(select nama from bahan where id = kb.id_bahan) bahan,(select s.nama from satuan s join bahan b on b.id_satuan = s.id where b.id = kb.id_bahan) satuan from keluar_bahan kb group by concat(MONTHNAME(waktu),' ',year(waktu)),kb.id_bahan");
			$data['waktu'] = DB::select("select concat(MONTHNAME(waktu),' ',year(waktu)) bulan,DATE_FORMAT(`waktu`,'%Y-%m') waktu from masuk_dana union select concat(MONTHNAME(waktu),' ',year(waktu)) bulan,DATE_FORMAT(`waktu`,'%Y-%m') from keluar_dana ORDER BY waktu asc");
			...
			$this->cbView('laporanbulanan',$data);
		}
		...
		public function getExport($filetype,$urut=""){
			...
			$data['bm'] = DB::select("select concat(MONTHNAME(waktu),' ',year(waktu)) waktu,sum(mb.jumlah) jumlah,(select nama from bahan where id = mb.id_bahan) bahan,(select s.nama from satuan s join bahan b on b.id_satuan = s.id where b.id = mb.id_bahan) satuan from masuk_bahan mb group by concat(MONTHNAME(waktu),' ',year(waktu)),mb.id_bahan");
			$data['bk'] = DB::select("select concat(MONTHNAME(waktu),' ',year(waktu)) waktu,sum(kb.jumlah) jumlah,(select nama from bahan where id = kb.id_bahan) bahan,(select s.nama from satuan s join bahan b on b.id_satuan = s.id where b.id = kb.id_bahan) satuan from keluar_bahan kb group by concat(MONTHNAME(waktu),' ',year(waktu)),kb.id_bahan");
			$data['waktu'] = DB::select("select concat(MONTHNAME(waktu),' ',year(waktu)) bulan,DATE_FORMAT(`waktu`,'%Y-%m') waktu from masuk_dana union select concat(MONTHNAME(waktu),' ',year(waktu)) bulan,DATE_FORMAT(`waktu`,'%Y-%m') waktu from keluar_dana ORDER BY waktu asc");
			$data['saldobank'] = DB::select("select IFNULL((select sum(jumlah) from masuk_dana where via = 'bank'),0) - ifnull((SELECT sum(jumlah) from keluar_dana where via = 'bank'),0) bank");
			...
			switch($filetype) {
				case "print":
					echo $view = view('xlaporanbulanan',$response)->render();
				break;
				case "pdf":
					$view = view('xlaporanbulanan',$response)->render();
